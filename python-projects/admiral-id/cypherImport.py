...
def generateCsvData(baseCsvFName, numCsvRows):
    ...
    with open(baseCsvFile) as sourceFile:
        with open(csvFile, 'w') as targetFile:
            for line in islice(sourceFile, (numCsvRows+1)): # add one to account for the header row
...
def getImportCypherFileName():
    # may want this to be more complex in the future ....
    cypherFName = "/root/dev/ARD-ADID/scripts/adid.cyp"
    return cypherFName

def getDeleteCypherFileName():
    # may want this to be more complex in the future ....
    cypherFName = "/root/dev/ARD-ADID/scripts/del.cyp"
    return cypherFName

def createCypherFile(templateFile, csvFile, batchSize):
   cypherFile = getImportCypherFileName()
...
# 1 - empty the DB
runBashCommand("cat " + getDeleteCypherFileName() + " | cypher-shell")
...
print("Recs: %8d   (BatchSize %5s) :: Import took : %8.1f s" % (csvRows, batchSize, elapsed))
