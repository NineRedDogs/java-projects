...
def oldRemoveDuplicateRows(dupFile):
    # we wan't to preserve header i.e. temporarily remove from sort
    print("removing duplicates for " + dupFile)
    os.system("head -n 1 " + dupFile + " > tmp;tail -n +2 " + dupFile + " | sort -u >> tmp; mv tmp " + dupFile)
...
def determineTotalScoreWeighting():
