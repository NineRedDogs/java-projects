...
# 192.168.2.204 is a vip alias - setup by running ifconfig eth0:0 192.168.2.204
...
ALIAS_ADDRESS=192.168.2.204

NODE_ADDRESS=192.168.2.18
...
iptables -t nat -A PREROUTING -p tcp --dport 80 -d $ALIAS_ADDRESS -j DNAT -m statistic --mode nth --every 2 --packet 0 --to-destination $NODE_ADDRESS:8001
iptables -t nat -A PREROUTING -p tcp --dport 80 -d $ALIAS_ADDRESS -j DNAT -m statistic --mode nth --every 1 --packet 0 --to-destination $NODE_ADDRESS:8002
